...
import { DriversDataDetail } from "../../features/lap-comparison/models/drivers-data-detail.model";
...
export class F1DataService {
  ...
  getDriverLapData(year: number, venue: string, sessionType: string, firstDriverNumber: number, firstLapNumber: number,
                   secondDriverNumber: number, secondLapNumber: number): Observable<DriversDataDetail> {
    const url = `${this.baseUrl}/driver_data/${year}/${venue}/${sessionType}/${firstDriverNumber}/${firstLapNumber}/${secondDriverNumber}/${secondLapNumber}/`;
    return this.http.get<DriversDataDetail>(url);
  }
}
